...
type AgletLoadProc = fn(name: &str) -> *mut std::ffi::c_void;
...
unsafe fn load_versions(load_proc: AgletLoadProc) -> Result<(), AgletError> {
    let local_glGetError_ptr = load_proc("glGetError");
    if local_glGetError_ptr.is_null() {
        return Err(AgletError::ProcLoad);
    }
    let local_glGetError: unsafe extern "C" fn() -> GLenum = unsafe {
        std::mem::transmute(local_glGetError_ptr)
    };

    #= if @{target_api_version_major} >= 3 =#
    let local_glGetIntegerv_ptr = load_proc("glGetIntegerv");
    if local_glGetIntegerv_ptr.is_null() {
        return Err(AgletError::ProcLoad);
    }
    let local_glGetIntegerv: unsafe extern "C" fn(GLenum, *mut GLint) = unsafe {
        std::mem::transmute(local_glGetIntegerv_ptr)
    };

    let gl_err = unsafe { local_glGetError() };
    if gl_err == GL_NO_ERROR {
        let mut ver_major: i32 = 0;
        let mut ver_minor: i32 = 0;

        unsafe { local_glGetIntegerv(GL_MAJOR_VERSION, &mut ver_major); }
        if unsafe { local_glGetError() } != GL_NO_ERROR {
            return Err(AgletError::GlError);
        }

        unsafe { local_glGetIntegerv(GL_MINOR_VERSION, &mut ver_minor); }
        if unsafe { local_glGetError() } != GL_NO_ERROR {
            return Err(AgletError::GlError);
        }
    ...
    let local_glGetString_ptr = load_proc("glGetString");
    if local_glGetString_ptr.is_null() {
        return Err(AgletError::ProcLoad);
    }
    let local_glGetString: unsafe extern "C" fn(GLenum) -> *const GLubyte = unsafe {
        std::mem::transmute(local_glGetString_ptr)
    };

    let ver_str: std::string::String = unsafe {
        let ver_c_str: *const std::ffi::c_uchar = local_glGetString(GL_VERSION);
        if local_glGetError() != GL_NO_ERROR || ver_c_str.is_null() {
            return Err(AgletError::GlError);
        }
        std::ffi::CStr::from_ptr(ver_c_str.cast()).to_string_lossy().to_string()
    };
...
unsafe fn load_extensions(load_proc: AgletLoadProc) -> Result<(), AgletError> {
    ...
    #= if @{target_api_version_major} >= 3 =#
    let local_glGetStringi_ptr = load_proc("glGetStringi");
    let local_glGetIntegerv_ptr = load_proc("glGetIntegerv");
    if !local_glGetStringi_ptr.is_null() && !local_glGetIntegerv_ptr.is_null() {
        let local_glGetStringi: unsafe extern "C" fn(GLenum, GLuint) -> *const GLubyte = unsafe {
            std::mem::transmute(local_glGetStringi_ptr)
        };
        let local_glGetIntegerv: unsafe extern "C" fn(GLenum, *mut GLint) = unsafe {
            std::mem::transmute(local_glGetIntegerv_ptr)
        };
        ...
        unsafe { local_glGetIntegerv(GL_NUM_EXTENSIONS, &mut num_exts); }
        ...
        if gl_err == GL_NO_ERROR {
            for i in 0u32..(num_exts as u32) {
                let cur_ext: std::string::String = unsafe {
                    std::ffi::CStr::from_ptr(local_glGetStringi(GL_EXTENSIONS, i).cast()).to_string_lossy().to_string()
                };
    ...
    let local_glGetString: fn(GLenum) -> *const GLubyte = unsafe {
        std::mem::transmute(local_glGetString_ptr)
    };

    let exts_str: std::string::String = unsafe {
        let exts_c_str: *const std::ffi::c_uchar = local_glGetString(GL_EXTENSIONS);
        std::ffi::CStr::from_ptr(exts_c_str.cast()).to_string_lossy().to_string()
    };
    let gl_err = unsafe { local_glGetError() };
    if gl_err != GL_NO_ERROR {
        return Err(AgletError::GlError);
...
#[allow(dead_code, non_snake_case)]
fn check_minimum_version() -> Result<(), AgletError> {
    if !unsafe { AGLET_@{min_api_version} } {
...
#[allow(dead_code, non_snake_case)]
fn check_required_extensions() -> Result<(), AgletError> {
    let mut missing_ext = false;

    #= foreach extensions =#
    if !unsafe { AGLET_@{name} } {
        if @{required} {
...
#[allow(dead_code, non_snake_case)]
fn load_procs(load_proc: AgletLoadProc) -> Result<(), AgletError> {
    unsafe {
        #= foreach procs =#
        aglet_@{name} = Some(std::mem::transmute(load_proc("@{name}")));
        #= /foreach =#
    }
